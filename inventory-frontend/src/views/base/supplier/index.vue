<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" border style="width: 100%">
      <el-table-column prop="supCode" label="编码" width="120" />
      <el-table-column prop="supName" label="供应商名称" min-width="150" />
...
<script>
import { getSupplierList, createSupplier, updateSupplier, deleteSupplier } from '@/api/base'
  ...
  methods: {
    ...
    handleSubmit() {
      this.$refs.form.validate(async valid => {
        ...
        try {
          if (this.form.id) {
            await updateSupplier(this.form)
    ...
    // 删除
    handleDelete(row) {
      this.$confirm('确认删除该供应商？删除后无法恢复', '提示', {
        type: 'warning'
      }).then(async () => {
        try {
